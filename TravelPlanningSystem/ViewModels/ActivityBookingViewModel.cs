...
public class ActivityBookingViewModel
{
    ...
    [Range(1, int.MaxValue, ErrorMessage = "Please choose an available session.")]
    [Display(Name = "Activity Session")]
    public int ActivitySessionId { get; set; }

    [Range(1, 500, ErrorMessage = "Number of participants must be between 1 and 500.")]
    ...
    [RegularExpression(@"^\d{10,11}$", ErrorMessage = "Phone number must contain 10 to 11 digits only.")]
    ...
    [Display(Name = "Have a Promo Code?")]
    public string? PromoCode { get; set; }
    public decimal DiscountAmount { get; set; } = 0;

    public bool HasInsurance { get; set; } = false;
    public bool HasEquipmentRental { get; set; } = false;
