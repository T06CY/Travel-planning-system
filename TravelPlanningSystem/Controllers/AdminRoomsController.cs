...
public class AdminRoomsController(AppDbContext context, IWebHostEnvironment environment) : Controller
{
    ...
    [HttpPost, ValidateAntiForgeryToken] public async Task<IActionResult> Create(HotelRoomFormViewModel model)
    {
        ...
        var room = Map(new HotelRoom(), model); context.HotelRooms.Add(room); await context.SaveChangesAsync(); await SavePhotos(room.HotelRoomId, model.Photos);
    ...
    [HttpPost, ValidateAntiForgeryToken] public async Task<IActionResult> Edit(int id, HotelRoomFormViewModel model)
    {
        if (id != model.HotelRoomId) return BadRequest(); var room = await context.HotelRooms.FindAsync(id); if (room is null) return NotFound();
        if (!ModelState.IsValid) return View(model); Map(room, model); await context.SaveChangesAsync(); await SavePhotos(id, model.Photos); return RedirectToAction(nameof(Details), new { id });
    }
    ...
    [HttpPost, ValidateAntiForgeryToken] public async Task<IActionResult> SetPrimaryPhoto(int id)
    ...
    [HttpPost, ValidateAntiForgeryToken] public async Task<IActionResult> DeletePhoto(int id)
    ...
    private static HotelRoom Map(HotelRoom r, HotelRoomFormViewModel m) { r.HotelName = m.HotelName.Trim(); r.RoomName = m.RoomName.Trim(); r.Destination = m.Destination.Trim(); r.Address = m.Address.Trim(); r.Description = m.Description.Trim(); r.PricePerNight = m.PricePerNight; r.Capacity = m.Capacity; r.TotalRooms = m.TotalRooms; r.StarRating = m.StarRating; r.Amenities = m.Amenities?.Trim(); r.IsFeatured = m.IsFeatured; r.IsActive = m.IsActive; return r; }
    private static HotelRoomFormViewModel ToForm(HotelRoom r) => new() { HotelRoomId = r.HotelRoomId, HotelName = r.HotelName, RoomName = r.RoomName, Destination = r.Destination, Address = r.Address, Description = r.Description, PricePerNight = r.PricePerNight, Capacity = r.Capacity, TotalRooms = r.TotalRooms, StarRating = r.StarRating, Amenities = r.Amenities, IsFeatured = r.IsFeatured, IsActive = r.IsActive };
